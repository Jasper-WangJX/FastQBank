...
| AI 模型 | 文本 **DeepSeek-V3**；视觉 **Gemini 2.0 Flash**（默认，有免费额度）/ **GPT-4o-mini**（备选）；统一 Provider 抽象 | 视觉模型按需调用做 OCR 兜底 + LaTeX，预算友好 |
...
**视觉模型**：**Gemini 2.0 Flash**（默认，最便宜档、个人使用有免费额度）/ **GPT-4o-mini**（备选），用于 `parse-question`：无标号选项拆分 + 公式 / LaTeX 识别（这是本地 PaddleOCR 解决不了的部分）。

**抽象层**：`LLMProvider` 接口，文本 / 视觉可分别配置、一键切换（OpenRouter / Qwen / GPT 等）。
...
- 云端同步（LWW + 软删除）
- JSON 导入导出
- AI 接入：文本（DeepSeek-V3）+ 按需视觉（Gemini 2.0 Flash / GPT-4o-mini）做 OCR 兜底与公式 / LaTeX 识别 + 服务端限流
