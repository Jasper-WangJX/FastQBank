...
| 10 Polish + Windows installer | ✅ Done (2026-05-21) | NSIS .exe installer, AC-logo icon pipeline, sonner toasts, offline banner, NSIS customization (auto-nest install dir + selectable shortcuts + uninstall cleanup), README rewrite, v1.0.0 |
...
> **Status: ✅ Done (2026-05-21).** Delivered via brainstorming → spec → plan → subagent-driven execution (task-by-task with two-stage spec/quality reviews). Final installer at `apps/desktop/release/FastQBank Setup 1.0.0.exe` (~310 MB). User-accepted smoke including OCR offline path, sonner toasts on network/5xx, offline banner, NSIS auto-nest + MessageBox shortcut choices. Spec at `docs/superpowers/specs/2026-05-20-phase10-windows-installer-and-polish-design.md`, plan at `docs/superpowers/plans/2026-05-20-phase10-windows-installer-and-polish.md`.

#### As-built notes (deviations from the original plan)

- **Onboarding REVERTED**: the spec's "lightweight empty-state onboarding" was implemented (`OnboardingEmpty.tsx` + `shouldShowOnboarding` predicate, TDD with 5 vitest cases) and then **reverted per user decision** for v1.0 (not wanted). The component files are gone; `QuestionListPage.tsx` matches `main` byte-for-byte for the affected region. Worth a follow-up consideration if external users start reporting "where do I begin" confusion.
- **Icon pipeline kept pure-zlib + one new dep (`png-to-ico`), no `sharp`** — the spec floated adding `sharp`, but the Phase 4 in-house PNG decoder + nearest-neighbor resizer already covered everything except ICO assembly. Source: `resources/fastqbICON.png` (810×810 square crop the user authored). Outputs: `icon.png` (256), `tray.png` (32), `icon.ico` (16/24/32/48/64/128/256 multi-resolution).
- **NSIS customization beyond the spec** (`apps/desktop/build/installer.nsh`): three additions requested by the user during smoke: (1) `.onVerifyInstDir` appends `\FastQBank` to any chosen install dir lacking that leaf; (2) `customInstall` MessageBox YesNo prompts for Desktop + Start Menu shortcuts (replacing electron-builder's flag-driven creation, which is now `false` in `package.json`); (3) `customUnInstall` `RMDir /r` on `%APPDATA%\FastQBank` / `%LOCALAPPDATA%\FastQBank`, guarded against empty env vars.
- **Token retention across uninstall/reinstall is currently accepted as v1.0 behavior** (NOT cleared). The `customUnInstall` macro runs without error during uninstall, but the Electron localStorage / IndexedDB store survives in practice — likely because Electron writes session data under a path the macro doesn't reach, or recreates it on next install. The user has chosen not to debug this for v1.0; documented here so a future "true clean uninstall" feature has the starting point.
- **`pnpm dist` initially broken on Windows cmd.exe** — the existing `build:sidecar` pnpm script `"../../packages/ocr-sidecar/.venv/Scripts/python.exe ..."` failed under pnpm's cmd.exe wrapper (cmd parses leading `..` as the command name). Fixed by introducing a Node shim (`apps/desktop/scripts/build-sidecar.cjs`) that resolves paths via `path.resolve(__dirname, ...)` and `spawnSync`s the python directly. Now `pnpm --dir apps/desktop dist` runs end-to-end as a single command.
- **Lint: 3 pre-existing `react-hooks/set-state-in-effect` errors** in `AuthContext.tsx` / `SettingsModal.tsx` / `OAuthCallbackPage.tsx` were detected on this branch — they also reproduce on `main`, predate Phase 10, and are flagged as a separate follow-up.
- **Sidecar packaged smoke (Phase 5 handoff)**: the `app.isPackaged` branch of `sidecar.ts` was exercised for the first time via the real NSIS installer; OCR end-to-end works offline through the installed app, no orphan `ocr_server.exe` after tray Quit.
- **Toast `position="top-right"` may collide visually with the Electron desktop's `WindowControls` cluster in the top-right of the custom title bar.** Toasts dismiss in ~4s and are not blocking; if it bothers users, switching to `top-center` is a one-character fix in `AppLayout.tsx`.
